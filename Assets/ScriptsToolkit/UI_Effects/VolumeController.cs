...
public class VolumeController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        volumeSlider.onValueChanged.AddListener(VolumeChanged);      //注册监听——滑条一变动就自动调用VolumeChanged(监听函数)
    }
...
// PlayerPrefs 是Unity内置控制的轻量级存档系统（键值存储）：只支持3种数据类型：int  float  string
// 其实也可以不要[ volumeSlider.onValueChanged.AddListener(VolumeChanged); ]这行代码：直接在Inspector里绑定函数VolumeChanged即可